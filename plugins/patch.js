  ...
  async (message, args) => {
    try {
      ...
      if (url.toLowerCase().includes("https")) {
        ...
        return await message.send("`FUTURE_MD-V2- UPDATE WAS SUCCESSFULL ✅`");
      } else {
...
${menuThemeFooter}                         
*┌┤✑  Thanks for Choosing FUTURE_MD-V2
│└────────────┈ ⳹        
...
astro_patch.smd(
  ...
  async (context) => {
    ...
    await context.send(
      `*FUTURE_MD-V2 ping is: ${pingTime} ᴍs*`,
      { edit: messageKey },
      "",
      context
    );
    ...
    try {
      ...
      const designs = [
        async () => {
          ...
          const captionText = `FUTURE_MD-V2 1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds${quoteText}\n\nFUTURE_MD-V2`;
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*FUTURE_MD-V2 ʀᴜɴɴɪɴɢ sɪɴᴄᴇ:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "FUTURE_MD-V2 Running Since",
        body: message,
        footerText: "FUTURE_MD-V2 2024",
    ...
    try {
      ...
      if (!isNaN(_0x56b5d3)) {
        if (
          ...
          _0x1eb88a[1][_0x210c92(362)]("FUTURE_MD-V2_FANCY_TEXT")
        ) {
